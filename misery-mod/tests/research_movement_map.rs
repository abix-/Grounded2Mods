...
fn read_movement_speeds_map() {
    ...
    let instances = research::walk_class_instances(&api, "BP_CharacterComponent_C", 100);
    ...
    let Some(inv_ptr) = research::read_component_ptr(&api, cc.addr, 0x218) else {
        println!("PlayerInventory pointer is null or read failed");
        return;
    };
    println!("PlayerInventory at addr:0x{inv_ptr:x}");

    let hdr_bytes = research::read_bytes(&api, inv_ptr, MOVEMENT_SPEEDS_MAP, 16);
    if hdr_bytes.len() < 16 {
    ...
    let elem_ptr = research::from_le_u64(&hdr_bytes, 0);
    let num = research::from_le_i32(&hdr_bytes, 8);
    let max = research::from_le_i32(&hdr_bytes, 12);
    ...
    for slot in 0..num as usize {
        ...
        let key = data[base];
        let value = research::from_le_f64(&data, base + 8);
        let hash_next = research::from_le_i32(&data, base + 16);
        let hash_idx = research::from_le_i32(&data, base + 20);
