...
| 1 | `ue::actor` | [ ] Make `on_each_load` apply once and stop, woken by the world going away, instead of searching every object every 3 seconds forever. Same pattern as the notice watcher and `spawning` | The `vendors` finder stops costing 1009 ms per 30 seconds, proven by `research_timing`. |
