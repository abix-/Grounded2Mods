...
| 1 | `vendors.rs` | [ ] Go to the main menu after a vendor pass and confirm no `FMallocBinned2` canary crash, now that BOTH the array growth and the per-entry price array use the engine allocator (research.md 27) | Operator reaches the main menu after a vendor pass with no crash. |
