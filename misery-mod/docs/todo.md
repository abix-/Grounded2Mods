...
| 1 | `spawning.rs` | [ ] Answer "which squares are loaded" from the generator's `StreamingLevels` and each entry's `LoadedLevel` at +0x158, instead of searching every object (worldgen.md 10) | A tick with no new square costs no object search, proven by `research_timing`. |
