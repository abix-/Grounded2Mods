...
| 1 | `ue::actor` | [ ] A live object found ONCE and kept: cache the pointer per class, drop it when `streaming::world_is_up` says the world went. Not a timer; time-based caching is polling with a longer gap | `find_actor` for the same class twice in a session does one object search, proven by `ue:objects_read` in `timing_report`. |
| 1 | `misery` | [ ] Point the five separate `find_actor` calls at the kept pointer: `speed.rs:34`, `shining.rs:69` and `:74`, `vendors.rs:231`, `lib.rs:155`. Three of them hunt the same player | A session does one search per class, not one per call. |
...
| 2 | `speed.rs` | [ ] Stop the tab reading at all: read the speeds once when the world loads, keep them, update them when the mod writes them. The mod is the only thing that changes them | Opening the Speed tab costs no object search and no read per frame. |
