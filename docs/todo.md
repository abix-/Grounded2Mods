...
| 1 | `modforge::input` | [ ] Make the bot output only virtual key state and relative mouse movement while travelling | The bot has no API that directly moves, rotates, or interacts with the player, and releases held keys on arrival, cancellation, and failure. |
...
| 1 | Modforge bot navigation | [ ] Define one shared path and player-observation format for Unreal and Unity | The same bot code accepts path points, player position, and view from either engine without engine-specific branches. |
...
| 1 | MISERY performance | [ ] Add a scoped timing guard around the complete route and enforce the performance document's acceptance gate | Timing disables after success or panic; `ue:find_object`, `ue:find_objects_by_chain`, `ue:find_actors_by_chain`, and `ue:objects_read` are absent; every measured bot operation remains below 16.7 ms. |
