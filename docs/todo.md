...
|---:|---|---|---|
| 1 | Modforge route | [ ] Replace the waypoint graph and its second A* with one ordered route | A route contains only ordered waypoints and actions; `RouteGraph`, `RouteEdge`, and waypoint-graph path search no longer exist. |
| 1 | Modforge bot navigation | [ ] Add separate shared path, path-point, and player-observation types | Unreal and Unity return the same path points plus player position, yaw, and pitch without representing path points as waypoints. |
| 1 | Modforge player input | [ ] Replace bot movement axes with virtual W/A/S/D key state and relative mouse movement | Bot commands contain only key state and mouse movement and release W/A/S/D on arrival, cancellation, stuck detection, and failure. |
| 1 | Modforge bot navigation | [ ] Replace the forbidden path-follower code and terminology with the shared bot | The bot reads a path, observes the player, and returns player-input commands without calling an engine or input implementation directly. |
| 1 | Modforge bot navigation | [ ] Prove Unreal and Unity receive identical commands for identical paths and observations | Shared tests pass the same path and player observations through both engine test implementations and receive the same ordered commands. |
| 1 | Player input source proof | [ ] Check the actual Ueforge and MISERY production sources for forbidden bot bypasses | The permanent test reads production files and rejects direct movement, look, interaction-handler, navigation-movement, and transform paths. |
| 1 | Ueforge input | [ ] Inject virtual W/A/S/D key state through Unreal's normal player input processing | The game moves the player through its existing key bindings without `SimpleMoveToLocation`, `AddMovementInput`, or direct transform writes. |
| 1 | Ueforge input | [ ] Inject virtual relative mouse movement through Unreal's normal player input processing | The game rotates the view through its existing mouse binding without `AddYawInput`, `AddPitchInput`, or physical cursor movement. |
| 1 | Ueforge input | [ ] Inject virtual `E` press and release through Unreal's normal player input processing | MISERY receives the same bound interaction input as a player without calling its interaction or Enhanced Input action handler directly. |
| 1 | MISERY navigation | [ ] Live-verify Unreal A* pathing and virtual player input from spawn through both doors | For each selected waypoint, Unreal A* returns a complete path from the player's current position and the bot travels it using only virtual W/A/S/D, mouse movement, and `E`. |
| 1 | Unityforge bot navigation | [ ] Return a complete Unity navigation path and current player position and view to Modforge | A Unity test supplies the same shared path and observation format used by the Unreal test. |
| 1 | Unityforge player input | [ ] Inject Modforge virtual W/A/S/D, relative mouse movement, and key press and release through Unity's normal player input | A Unity test receives the same ordered player-input commands as Unreal without direct movement, rotation, or interaction calls. |
