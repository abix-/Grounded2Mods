...
|---:|---|---|---|
| 1 | `modforge::route` | [ ] In a live MISERY expedition, discover the nearest placed loot box, make that target the only new semantic waypoint, use Unreal navigation for the detailed path selected by Modforge A*, interact with the box like the player, and take its contents. | A permanent cold live flow enters the expedition, selects a real nearby loot box without a hand-authored position, saves a graph whose only post-entry waypoint is that loot-box target, follows the A* result, opens the box, transfers its loot, and stops cleanly with observed evidence that the box was looted. |
