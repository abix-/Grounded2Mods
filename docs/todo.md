...
|---:|---|---|---|
| 1 | MISERY control plane | [ ] Expose MISERY's retained current-world player pointer to live tests and enumerate expedition and metal doors through Unreal's actor collections | Cold route setup obtains the player and both doors without `find_live_instance`, `walk_class_instances`, `walk_class_chain_instances`, or any global UObject scan. |
| 1 | MISERY navigation | [ ] Remove scan-based nearby-actor discovery from stuck handling and diagnose only the retained blocking door | Reaching the bunker door performs no `walk_class_chain_instances` calls and logs only the retained door's state, bounds, range, and interaction result. |
| 1 | MISERY navigation | [ ] Observe expedition entry through the retained player pointer | The entry wait reads that player's location directly and contains no `find_live_instance` polling. |
| 1 | MISERY diagnostics | [ ] Remove or convert the obsolete scan-based navigation and looting diagnostics | `research_navigation.rs` contains no calls to `find_live_instance`, `walk_class_instances`, or `walk_class_chain_instances`. |
...
| 1 | `modforge::route` + MISERY navigation | [ ] Traverse ranked crate candidates until one succeeds and save only that crate as the post-entry waypoint | Invalid, partial, and observed-stuck candidates are rejected, then the first traversed crate produces one A* edge and one new waypoint. |
