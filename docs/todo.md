...
| 5 | `modforge` | [ ] Lift Survivalist's named-unique registry, entered set, holder ledger, schema, and persistence from survivalist-mod/src/unique.rs into a reusable modforge module; keep prototype lookup, inventory scans, item delivery, and chronicle text in survivalist-mod | Survivalist uses the modforge unique tracker and its mod contains only Survivalist-specific bindings. |
| 5 | `modforge` | [ ] Lift Survivalist's rectangle and annex geometry from survivalist-mod/src/development.rs into modforge settlement planning with a caller-supplied terrain-passability predicate | Annex planning is unit-tested in modforge; Survivalist only reads Unity state and applies the returned plan. |
| 5 | `modforge` | [ ] Move the generic cadence gate out of modforge::mission into a general timing module and replace Survivalist's remaining hand-rolled AtomicU32/f32 tick gates | Survivalist has one shared cadence implementation and no repeated timestamp-bit gate logic. |
| 5 | `modforge` | [ ] Expand modforge::quality with reusable tier definitions, sibling count, source odds, and skill-scaled crafting policy; keep Harmony hooks, prototype lookup, inventory walking, and item swaps in survivalist-mod | Survivalist quality balance is declarative modforge data and the mod contains only game bindings. |
| 5 | `modforge` | [ ] Lift the engine-agnostic settlement-upgrade model from unityforge/cs-shim-survivalist/Upgrades.cs: track definitions, per-entity levels, costs, diminishing curves, schema, and persistence; keep Survivalist Harmony effects in the shim | modforge is the authority for upgrade state and policy; the Survivalist shim only binds levels to game behavior. |
| 5 | `modforge` | [ ] Extend modforge::mission for the lifecycle shapes still hand-driven by Survivalist courier, settler, stranger, and robbery systems, then migrate those systems | All Survivalist mission transitions, timeout handling, and cleanup use shared mission controllers. |
