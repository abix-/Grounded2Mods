...
| 1 | `modforge::input` | [ ] Make the shared follower output only virtual key state and relative mouse movement | Path following has no API that directly moves, rotates, or interacts with the player, and releases held keys on arrival, cancellation, and failure. |
| 1 | MISERY navigation | [ ] Live-verify A* route selection, Unreal path points, and virtual player-input following from spawn through both doors | A restarted run reaches and enters the expedition using only virtual W/A/S/D, mouse movement, and `E`, with input released at both stops and on failure. |
