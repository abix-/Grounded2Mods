...
| 1 | MISERY player input | [ ] Move the character via Enhanced Input InjectInputForAction | Call `UEnhancedInputLocalPlayerSubsystem::InjectInputForAction(ForwardInput action, FInputActionValue(1.0), [], [])` each frame via the `call` op; the character walks while injected and stops when injection stops. Prior art: Lyall/WukongTweak SDK. |
