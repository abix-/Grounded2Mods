...
| 1 | MISERY player input | [x] Find InputKey and call it to inject a virtual W press | SOLVED via prior art (bl-sdk): InputKey is EnhancedPlayerInput vtable slot 88 (RVA + 0x42f5970), called as `vtable[88](epi, &FInputKeyParams)` with the correct struct layout and a real FKey (valid FKeyDetails, lifted from an EPI block). Proven by `find_inputkey_slot_via_iskeydown`: after the call IsInputKeyDown(W)=true, after release=false. research.md section 31. |
