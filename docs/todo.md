...
| 1 | MISERY input | [ ] Make crate aiming change the player's control rotation when the viewport has lost mouse capture | Each look step uses the player controller's yaw and pitch input, observes rotation progress, and reaches the crate aim without depending on foreground mouse capture. |
