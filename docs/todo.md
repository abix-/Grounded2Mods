...
|---:|---|---|---|
| 1 | Ueforge input | [ ] Delete the rejected Unreal console-command input implementation | `Input.+key`, `Input.-key`, and `KismetSystemLibrary.ExecuteConsoleCommand` are absent from Ueforge player input, and the workspace compiles with player input explicitly unavailable until the real path lands. |
| 1 | MISERY input proof | [ ] Add a permanent live W press-and-release test before implementing the replacement | The ignored live test fails unless W changes the retained player's position and releasing W stops movement. |
...
| 1 | MISERY input proof | [ ] Add and run a permanent live relative-mouse test | The test observes the retained player's view before and after input and fails unless the requested mouse movement changes it. |
| 1 | Ueforge input | [ ] Send E press and release through the same Unreal player-input path | Ueforge sends the bound E key through the retained local player's input without calling a MISERY interaction function or input-action handler. |
| 1 | MISERY input proof | [ ] Add and run a permanent live E interaction test at a door | The test aims at an in-range door, sends E through player input, and fails unless MISERY's existing binding changes the observed door state. |
...
| 1 | MISERY input proof | [ ] Reject every player-input bypass in permanent production-source tests | The test rejects Unreal console commands, direct movement, direct rotation, direct interaction handlers, player transform writes, physical cursor movement, and OS-wide input. |
