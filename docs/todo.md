...
|---:|---|---|---|
| 1 | `modforge::route` | [ ] Distill the live MISERY spawn-to-expedition proof into exactly three durable stop waypoints: spawn, metal door, and expedition door. Attach a conditional open action to the bunker metal door and an enter-expedition interaction to the expedition door. Keep dense position samples as diagnostic breadcrumbs, never graph nodes. | The saved graph has exactly three waypoints and two edges; Modforge A* sees only the stop graph; Unreal navigation owns detailed movement along each edge; a cold live replay opens the bunker door only when closed, uses the expedition door, enters the expedition, and stops cleanly. |
