...
|---:|---|---|---|
| 1 | Ueforge input | [ ] Read yaw through the reflected `AController.ControlRotation` field instead of calling the unavailable `GetControlRotation` function | A live MISERY follower observes yaw and sends movement without a missing-function error. |
