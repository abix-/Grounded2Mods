...
|---:|---|---|---|
| 1 | MISERY navigation | [ ] Make the cold three-stop route reliably open the bunker metal door through player `E` input | Three consecutive restarted runs leave the bunker and reach the expedition door without manual input. |
| 1 | MISERY navigation | [ ] Retain the player and expedition state across entry instead of polling the global UObject list | Expedition entry is observed from retained pointers and the live flow performs no global object search after entering. |
...
| 1 | MISERY performance | [ ] Add a scoped timing guard and enforce the zero-scan frame budget for the loot flow | Timing always disables after success or panic, forbidden scan counters stay absent after entry, and every measured bot operation remains below 16.7 ms. |
