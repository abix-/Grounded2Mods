...
|---:|---|---|---|
| 1 | `misery-mod` / `ueforge` | [ ] Generate the spawn-to-expedition waypoints from MISERY's Unreal navmesh instead of requiring a manual recording. Live evidence finds the player and placed expedition door, but the assumed `FindPathToLocationSynchronously` parameter layout times out the game-thread control queue. Add live UFunction parameter offsets, build the call from that layout, reject invalid or partial paths before movement, and feed the returned path points to the one route follower. | From a loaded safe-area save, the permanent live test extracts a valid complete navmesh path, reports its waypoint chain, walks to the expedition door without teleporting, and releases all held input on success or failure. |
