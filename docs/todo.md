...
|---:|---|---|---|
| 1 | `modforge::route` + `misery-mod` | [ ] Record a live MISERY trail as world-space waypoints and replay it with closed-loop movement and relative mouse steering. The route graph owns A* choice among recorded traversable edges; the follower is the only writer of movement and look input while active; the live journal remains the sequence and observed-completion authority. Current bypasses are raw timed input and direct memory movement. | A permanent live test records a short route from the current player position, saves and loads it, returns to the start, replays the saved route without teleporting, returns again, releases every held key, and reports the exact waypoint and last pose if stuck. Unit tests prove trail reduction, schema round-trip, A* alternate-edge choice, shortest-turn steering, arrival, and stuck detection. |
