...
use crate::common::{ctype, display_name, for_each_community, handle_of, list_len, own};
...
fn with<R>(h: i32, f: impl FnOnce(&MonoObject) -> R) -> R {
    let o = own(h);
    let r = f(&o);
    std::mem::forget(o);
    r
    ...
    match m.stage {
        Stage::Going => {
            ...
            m.stolen = with(m.target_h, |t| {
                crate::predation::carry_off_stored_goods(t, &[m.thief_h], STEAL_MAX_STACKS)
            })?;
