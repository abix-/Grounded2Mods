...
1. More to do (ecosystem-generated work)  <- UP NEXT
2. Quality system (higher quality, lower chance)
3. More to discover (named uniques)
4. More to learn (rare people and knowledge)
5. Settlement upgrades (incremental, a resource sink)
...
| Settlement upgrades (incremental, a resource sink) | Gameplay | 0/10 | DESIGNED 2026-07-11 (operator vision), nothing built. The player's community must be able to grow and defend itself better, with HOURS of gameplay in the upgrading itself: every structure improvable with materials (more hit points, more storage capacity, added features), INCREMENTAL upgrades that keep going until the costs outweigh the benefits, which makes this the RESOURCE SINK for a player who gets too rich. What the engine already gives (prior audits): construction is recipe-driven with REAL ingredient consumption (research.md "Construction system"); buildings track damage per instance and camps keep repair lists; structures and recipes are story XML the mod can ship like items and quests; the quality generator's DRY pattern (leveled variants from a config of multipliers) applies directly. Open research, next: where a structure's max hit points and a container's capacity live (instance vs type), whether a placed structure can become its upgraded type IN PLACE (keeping position and contents), and which in-game affordance triggers an upgrade (the hard question). | The player can sink hours and endless materials into their settlement: every structure family has an upgrade ladder with escalating costs and real benefits (hit points, capacity, features), upgrades happen in place through an in-game affordance, a rich player always has something worth spending on, and the same ladders are open to AI camps so symmetry holds. |
