...
ALL of these are now BUILT (code-landed 2026-07-07), none
live-verified: the dread loop (telegraph, delay, false-alarm or
foreshadowed payoff, escalation) in incursion.rs, with payoff kinds
for the mega-horde, off-map raiders, military remnants, the settling
faction (claims a dead base via the game's own reclamation), the
mysterious stranger, the refugee wave (arms a foreshadow so what
they fled arrives on the next sign), and the off-map signal. What
remains is VERIFICATION, not building: the two foundations under the
hostile payoffs are unverified (the horde firing, which needs the
game restart, and a hostile group actually attacking rather than
fleeing). One restart plus one watched aggressive arrival unlocks
the scored credit for the whole set.
...
|---|---|---|
| Traveling mega-horde | 9/10 | BUILT (code-landed); holds until the game restart arms the spawner, then unverified |
| Off-map raiders / warband | 8/10 | BUILT (code-landed); rests on the unverified hostile-attack path (a group may flee, not fight) |
| Military remnants (special forces) | 8/10 | BUILT (code-landed); hostile to every camp, same unverified attack path |
| A settling faction | 7/10 | BUILT (code-landed); claims a dead base via the game's own reclamation (no create-from-nothing needed) |
| The mysterious stranger | 6/10 | BUILT (code-landed); a lone figure, outcome real but never explained |
| Refugee wave fleeing something | 5/10 | BUILT (code-landed); arms a foreshadow so a real threat follows |
| Off-map signal | 5/10 | BUILT (code-landed); chronicle lure, plague half dropped (no-infections directive stands) |
| Dread signs (the telegraph) | 4/10 raw, but core of the loop | BUILT (code-landed); the loop that carries every payoff above |
