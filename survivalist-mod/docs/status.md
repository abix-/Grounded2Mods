...
|---|---|---|---|---|
| Storyteller / director | Gameplay | 2/10 (proposed) | The explicit anti-boredom layer, and the newest idea here, so the least built. What exists is state-driven agent behavior (the desperation ladder: each settlement reads its own hunger, population, and threat and acts at the matching intensity) and one adaptive-pressure mechanic (the horde, below). There is NO director layer that reads world-and-player variables and paces events toward a tension curve. Next: define the director as a real layer (what variables it watches, what it can inject, how it targets rising-then-easing tension) with the horde recorded as its first instance, taking care it never flattens the world's own emergent behavior. | A director sits above the simulation, reads how well things are going and how long since anything happened, and paces events and pressure so the game stays unpredictable and dramatic, scaling to success, without smothering the emergent world underneath. |
| The horde (adaptive pressure) | Gameplay | 3/10 (proposed) | Operator-locked as the counterweight to the alpha settlement (the Mario Kart rule: first place should hurt). The biggest camp draws scaled zombie packs, tiered by its size, so growth is a real tradeoff and a snowballing winner must keep winning sieges or bleed. SHIPPED in code: every 5 minutes the largest camp above 16 members (player included) draws a size-tiered pack from the game's own spawner; at most two packs roam at once; the chronicle announces the massing; carried by bridge ABI v6. But it has NEVER run live: it holds back until the first game restart that loads the v6 shim, then arms. By the live-only rule it scores low until watched. Next: restart to arm it, then watch a pack actually mass on the alpha and the walls answer. | The alpha camp visibly draws stronger and more frequent zombie packs as it grows; the biggest faction is not automatically the safest; success buys danger, watched over real time. |
