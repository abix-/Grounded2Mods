...
| ScheduleOne.Map.Map exists with fields Regions + RegionDict, sibling type MapRegionData, region enum EMapRegion | cited | metadata string scan of interop Assembly-CSharp.dll 2026-08-07 (type + NativeFieldInfoPtr names) | live walk_class + inspect (tests/research_map.rs) |
| ScheduleOne.Cartel.CartelInfluence tracks per-region influence: GetInfluence(EMapRegion), ChangeInfluence, GetAllRegionInfluence -> RegionInfluenceData[], fields RegionInfluence + DefaultRegionInfluence | cited | same metadata scan; method signatures from NativeMethodInfoPtr names | live walk + inspect; observe influence change in-game |
