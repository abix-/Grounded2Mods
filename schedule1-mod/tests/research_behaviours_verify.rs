        ...
        for (i, inst) in list.iter().enumerate().take(check_count) {
            if let Some(h) = inst["handle"].as_i64() {
                            ...
                            if item.ok {
                                if let Some(ih) = handle_of(&item.result) {
                                    let insp = api.op("inspect_object",
                                        json!({"handle": ih}));
                                    let tn = insp.result["type"]
                                        .as_str().unwrap_or("?").to_string();
                                    let pri = api.op("read_field",
                                        json!({"handle": ih, "field": "Priority"}));
                                    let short = tn.rsplit('.').next()
                                        .unwrap_or(&tn).to_string();
                                    enabled_types.push(format!("{}(pri={})",
                                        short, pri.result));
                ...
                if let Some(ah) = handle_of(&active.result) {
                    ...
                    let tn = insp.result["type"].as_str()
                        .unwrap_or("?").to_string();
                    active_type = tn.rsplit('.').next()
                        .unwrap_or(&tn).to_string();
                    let pri = api.op("read_field",
                        json!({"handle": ah, "field": "Priority"}));
                    active_pri = pri.result.to_string();
                    api.op("release_handle", json!({"handle": ah}));
                }

                let type_short = npc_type.rsplit('.').next()
                    .unwrap_or(&npc_type);
                println!("  [{i}] {npc_name} ({type_short}) enabled={enabled_count} active={active_type}(pri={active_pri}) enabled_list=[{}]",
                    enabled_types.join(", "));
                            ...
                            if let Some(n) = count_of(&api, oh) {
                                ...
                                for i in 0..n {
                                    ...
                                    if let Some(ih) = handle_of(&item.result) {
                                        ...
                                        let pname = name.result.as_str()
                                            .or_else(|| name.result.get("str").and_then(|s| s.as_str()))
                                            .unwrap_or("?");
                                        if pname.contains("BaseEmployee") || pname.contains("Employee") {
                                                    ...
                                                    if comps.ok {
                                                        if let Some(ch) = handle_of(&comps.result) {
                                                            if let Some(cn) = count_of(&api, ch) {
                                                                println!("  {cn} components on {pname}:");
                                                                for ci in 0..cn {
                                                                    let comp = api.op("invoke_method",
                                                                        json!({"handle": ch, "method": "get_Item", "args": [ci]}));
                                                                    if !comp.ok { continue; }
                                                                    if let Some(cih) = handle_of(&comp.result) {
                                                                        let insp = api.op("inspect_object",
                                                                            json!({"handle": cih}));
                                                                        let tn = insp.result["type"]
                                                                            .as_str().unwrap_or("?");
                                                                        if tn.contains("Behaviour")
                                                                            || tn.contains("Behavior")
                                                                            || tn.contains("Idle")
                                                                        {
                                                                            println!("    [{ci}] {tn}");
                                                                        }
                                                                        api.op("release_handle", json!({"handle": cih}));
                                                                    }
                                                                }
                                                            }
                                                        ...
                                                        println!("  GetComponents failed: {:?}", comps.error);
                                                    }
            ...
            if let Some(h) = inst["handle"].as_i64() {
                ...
                if go.ok {
                    if let Some(gh) = handle_of(&go.result) {
                                    ...
                                    if let Some(pth) = handle_of(&parent.result) {
                                        ...
                                        if pgo.ok {
                                            if let Some(pgh) = handle_of(&pgo.result) {
                                                ...
                                                parent_name = pn.result.as_str()
                                                    .or_else(|| pn.result.get("str").and_then(|s| s.as_str()))
                                                    .unwrap_or("?").to_string();
                        ...
                        println!("  [{i}] NPC={npc_name} IdleBehaviour.gameObject={go_name} parent={parent_name}");
                        api.op("release_handle", json!({"handle": gh}));
                    }
                }
                api.op("release_handle", json!({"handle": h}));
        ...
        for (i, inst) in list.iter().enumerate().take(check_count) {
            if let Some(h) = inst["handle"].as_i64() {
                ...
                if let Some(ah) = handle_of(&active.result) {
                    ...
                    let insp = api.op("inspect_object", json!({"handle": ah}));
                    active_name = insp.result["type"].as_str()
                        .unwrap_or("?")
                ...
                if !matches {
                    consistent = false;
                    println!("  [{i}] MISMATCH: max_enabled_pri={:?} active_pri={:?} active={}",
                        max_pri, active_pri, active_name);
                }
                if i < 5 || !matches {
                    println!("  [{i}] max_enabled_pri={:?} active_pri={:?} active={}{}",
                        max_pri, active_pri, active_name,
