...
| 2 | `cs-shim-mono` | [ ] Install BepInEx 5 into the game directory and deploy the cs-shim-mono plugin so it loads the Rust cdylib at game startup | The game launches with BepInEx and the shim logs "Unityforge.Shim: ready" in the BepInEx log. |
| 3 | `fish-mod` | [ ] Build the cdylib in release, copy it next to the shim as fish_mod.unityforge.dll, and confirm the HTTP control plane answers on the chosen port | `curl localhost:<port>/op` with a ping op returns a success response while the game runs. |
