...
| 5 | `modforge` | [ ] Add `AbstractItem` struct with name, type, subtype, quality_tier, value, weight, generic properties map, origin_game, and origin_data blob. Add inventory helpers: `add_item()`, `remove_item()`, `items_by_type()` | Abstract items serialize/deserialize through the profile, and a round-trip test passes |
| 20 | first game mod | [ ] Implement `to_abstract(game_item)` and `from_abstract(abstract_item)` for one game mod. Round-trip through the same game must be lossless via origin_data | Converting an in-game item to abstract and back produces the identical in-game item |
