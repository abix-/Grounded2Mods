...
| I-10: shared player-input route execution | in progress | `InputSurface` now carries forward, right, yaw, pitch, and key commands. The shared path follower consumes engine path points and observed poses, then emits ordered player commands with neutral movement on arrival, cancellation, or stuck detection. Remaining: Unreal adapter and MISERY migration. |
